...
<div class="relative w-full min-h-screen flex items-center justify-center px-4 sm:px-6 py-10">
    <div class="faq-container relative bg-black text-white px-6 sm:px-10 py-6 sm:py-8 rounded-2xl shadow-xl w-full max-w-2xl sm:max-w-3xl lg:max-w-4xl">
        <div class="absolute inset-0 rounded-2xl border-2 sm:border-4 border-blue-400 shadow-[0_0_20px_rgba(0,0,255,0.8)]"></div>

        <div class="relative">
            <h2 class="text-2xl sm:text-3xl font-bold text-center mb-4 sm:mb-6 text-transparent bg-clip-text bg-gradient-to-r from-purple-400 via-pink-300 to-purple-400 animate-pulse">Frequently Asked Questions</h2>
            
            {#each faqs as faq, i}
                <!-- svelte-ignore a11y_no_static_element_interactions -->
                <!-- svelte-ignore a11y_click_events_have_key_events -->
                <div class="faq-item border-b border-gray-300 py-6 md:py-7 px-3 cursor-pointer transition duration-300 hover:bg-gray-800 hover:text-purple-400"  
                     on:click={() => toggle(i)}>
